...
在无网络且应用已被清理的情况下，可以调用 [login](@login) 接口时，[ZIMLoginConfig](@-ZIMLoginConfig) 中的 [isOfflineLogin](@isOfflineLogin) 传入 `true`，实现使用上一次的用户信息离线登录 App。当通过 [ZIMLoggedInCallback](@-ZIMLoggedInCallback) 回调确定登录成功后，即可访问本地 SDK 数据。 
