...
当 [onRecorderCompleted](https://pub.dev/documentation/zego_zim_audio/latest/zego_zim_audio/ZIMAudioEventHandler/onRecorderCompleted.html) 触发后，消息发送客户端可以使用音频文件绝对路径构造 [ZIMAudioMessage](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMAudioMessage-class.html)（ZIM 音频消息），并调用 [sendMessage](@sendMessage) 接口发送该消息，下列代码以在单聊会话中发送音频消息为例。

```dart
// 录制完成回调
ZIMAudioEventHandler.onRecorderCompleted = (int totalDuration) {
    int second = totalDuration ~/ 1000; // 毫秒转秒

    // 构造 ZIM 音频消息
    ZIMAudioMessage audioMessage =
        ZIMAudioMessage('录制音频文件绝对路径', audioDuration: second);
    ZIMMessageSendConfig sendConfig = ZIMMessageSendConfig();

    // 用 sendMessage 发送，单聊传 ZIMConversationType.peer
    ZIM.getInstance()!
        .sendMessage(audioMessage, '单聊的会话ID', ZIMConversationType.peer, sendConfig)
        .then((ZIMMessageSentResult result) {
            // 发送成功
        })
        .catchError((error) {
