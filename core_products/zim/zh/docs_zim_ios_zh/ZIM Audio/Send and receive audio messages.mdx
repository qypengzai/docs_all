...
- (void)onRecorderCompleted:(int)totalDuration {
    ...
    // 构造 ZIM 音频消息
    ZIMAudioMessage *audioMessage = [[ZIMAudioMessage alloc] initWithFileLocalPath:@"录制音频文件绝对路径"
                                                                     audioDuration:second];
    ZIMMessageSendConfig *sendConfig = [[ZIMMessageSendConfig alloc] init];
    // 用 sendMessage 发送，单聊传 ZIMConversationTypePeer
    [[ZIM getInstance] sendMessage:audioMessage
                    toConversationID:@"单聊的会话ID"
