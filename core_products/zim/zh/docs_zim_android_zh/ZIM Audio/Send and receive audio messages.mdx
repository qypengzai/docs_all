...
ZIMAudio.getInstance().setEventHandler(new ZIMAudioEventHandler() {
    ...
    public void onRecorderCompleted(int totalDuration) {
        ...
        ZIM.getInstance().sendMessage(audioMessage, "单聊的会话ID",
                ZIMConversationType.PEER, sendConfig, new ZIMMessageSentFullCallback() {
            @Override
            ...
            @Override public void onMediaUploadingProgress(ZIMMediaMessage message, long currentFileSize, long totalFileSize) {}
            @Override public void onMultipleMediaUploadingProgress(ZIMMultipleMessage message, long currentFileSize, long totalFileSize) {}
        });
    }
});
```
