...
发送者发送回执消息后，若接收端设备已收到该消息，那么消息的回执状态将变更为已送达。开发者可通过 [onMessageReceiptChanged](@onMessageReceiptChanged) 回调监听回执状态的变化。

<Warning title="注意">
ZIM 3.1.0 版本起，消息回执在 “回执中” 和 “已完成” 之间新增了 “已送达” 状态，用于表示回执消息已送达。如需使用此功能，请联系 ZEGO 技术支持。
</Warning>

以下场景的消息回执状态会变更为 “已送达” 并触发 [onMessageReceiptChanged](@onMessageReceiptChanged) 回调。
...
调用 [queryGroupMessageReceiptMemberList](@queryGroupMessageReceiptMemberList) 可查询群消息的已读、未读（未送达）、或已送达状态的成员列表。
