...
In the case of no network and the app being cleared, you can call the [login](@login) interface with the [isOfflineLogin](@isOfflineLogin) parameter set to `true` in [ZIMLoginConfig](@-ZIMLoginConfig) to achieve offline login using the user information from the last login. After successful login, you can access the local SDK data.
