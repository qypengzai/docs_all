...
2. Trigger the callback.
   ...
   - After the recording starts, listen for the [recorderStarted](@recorderStarted) callback on the sending client to update the UI.
...
1. To complete the recording, call the [completeRecord](@completeRecord) method.
   ...
   <Note title="Note">
   - Make sure that you have received the [recorderStarted](@recorderStarted) callback before calling the [completeRecord](@completeRecord) method; otherwise, the recording is canceled, and an error is reported, indicating that the duration is too short.
...
After the [onRecorderCompleted](@onRecorderCompleted) callback is triggered, build the [ZIMAudioMessage](@-ZIMAudioMessage) object (ZIM audio message) by using the specified absolute path and call the [sendMessage](@sendMessage) method to send the message. Below is the sample code for sending an audio message in a one-to-one chat.
