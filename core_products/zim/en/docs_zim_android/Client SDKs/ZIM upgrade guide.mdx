...
The deprecated `sendPeerMessage`, `sendRoomMessage`, `sendGroupMessage`, and old `sendMessage` API (without [ZIMMessageSendNotification]() parameter) have been removed. Please use the [sendMessage](@sendMessage) API with the `notification` parameter instead.
...
- The `message` parameter in [onMediaDownloadingProgress]() has changed from `ZIMMediaMessage` to `ZIMMessage`.
</div>
:::
:::if{props.platform="window"}
