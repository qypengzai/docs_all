...
Relevant interfaces: [SendMessage](@SendMessage), [SendMessage](@SendMessage)
</td>
</tr>
<tr data-row-level="5">
...
To send rich media content after login, call the [SendMessage](@SendMessage) method, and specify the message type (image, file, audio, video), the session type (one-on-one chat, room chat, group chat), and message related configurations as needed.
