...
After the [onRecorderCompleted](https://pub.dev/documentation/zego_zim_audio/latest/zego_zim_audio/ZIMAudioEventHandler/onRecorderCompleted.html) callback is triggered, build the [ZIMAudioMessage](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMAudioMessage-class.html) object (ZIM audio message) by using the specified absolute path and call the [sendMessage](@sendMessage) method to send the message. Below is the sample code for sending an audio message in a one-to-one chat.
...
ZIMAudioEventHandler.onRecorderCompleted = (int totalDuration) async{
    ...
    ZIM.getInstance()!
        .sendMessage(audioMessage, 'toConversationID', ZIMConversationType.peer, sendConfig)
