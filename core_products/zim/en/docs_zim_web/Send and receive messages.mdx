...
For a detailed introduction to the interfaces, please refer to [onConnectionStateChanged](@onConnectionStateChanged), [onMessageReceived](@onMessageReceived), [tokenWillExpire](@tokenWillExpire).
...
After client B logs in, he will receive client A's message through the callback [on](@on) which is already set in the [onMessageReceived](@onMessageReceived) method. 
