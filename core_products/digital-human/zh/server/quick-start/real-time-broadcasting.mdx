...
播放端使用 ZEGO Express SDK 拉流播放，详情请参考各端（[Web](/real-time-voice-web/quick-start/implementing-voice-call) 、[Android](/real-time-voice-android/quick-start/implementing-voice-call) 、[iOS](/real-time-video-ios-oc/quick-start/implementing-video-call)）实现视频通话文档 。
