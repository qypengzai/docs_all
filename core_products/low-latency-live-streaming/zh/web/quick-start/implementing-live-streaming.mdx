...
1. 创建并初始化一个 [ZegoExpressEngine](@-ZegoExpressEngine) 的实例，将您项目的 AppID 传入参数 “appID”，Server 传入参数 “server”。
...
2. SDK 提供如房间连接状态、音视频流变化、用户进出等通知回调。创建引擎后，您可以通过引擎实例的 [on](@on) 方法注册回调。
...
- [camera](@camera-zegostreamoptions)：摄像头麦克风采集流相关配置

- [screen](@screen-zegostreamoptions)：屏幕捕捉采集流相关配置

- [custom](@custom-zegostreamoptions)：第三方流采集相关配置
</Accordion>
