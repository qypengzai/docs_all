...
在调用 [startPublishingStream](@startPublishingStream) 之前，调用 [setVideoConfig](@setVideoConfig) 接口，设置 [ZegoVideoConfig](@-ZegoVideoConfig) 类中的参数 `codecID` 为 `ZEGO_VIDEO_CODEC_ID_H264_DUAL_STREAM`，开启大小流视频编码；并调用 [startPublishingStream](@startPublishingStream) 接口开始推流。
