...
在调用 [startPublishingStream](@startPublishingStream) 之前，调用 [setVideoConfig](@setVideoConfig) 接口，设置 [ZegoVideoConfig](@-ZegoVideoConfig) 类中的参数 `codecID` 为 `ZegoVideoCodecIDSVC`，开启分层视频编码；并调用 [startPublishingStream](@startPublishingStream) 接口开始推流。
...
在调用 [startPublishingStream](@startPublishingStream) 之前，调用 [setVideoConfig](@setVideoConfig) 接口，设置 [ZegoVideoConfig](@-ZegoVideoConfig) 类中的参数 `codecID` 为 `ZegoVideoCodecIDH264DualStream`，开启大小流视频编码；并调用 [startPublishingStream](@startPublishingStream) 接口开始推流。
