...
在调用 [StartPublishingStream](@StartPublishingStream) 之前，调用 [SetVideoConfig](@setVideoConfig) 接口，设置 [ZegoVideoConfig](@-ZegoVideoConfig) 类中的参数 `codecID` 为 `ZegoVideoCodecIDH264DualStream`，开启大小流视频编码。
