...
3. 媒体播放器播放在线资源时，支持缓存资源文件
    ...
    相关 API 请参考 [ZegoMediaPlayerResource > onlineResourceCachePath](@onlineResourceCachePath), [ZegoMediaPlayerResource > maxCachePendingLength](@maxCachePendingLength-ZegoMediaPlayerResource)
...
1. 媒体播放器播放透明特效时，支持 Alpha 数据布局在 RGB 数据右上方
    ...
    媒体播放器播放透明特效时，新增 [ZegoAlphaLayoutType > ZegoAlphaLayoutTypeRightTop](@ZegoAlphaLayoutTypeRightTop)  枚举，以支持 Alpha 数据拼接在 RGB 数据右上方，设置此枚举时，仅支持 0.5x 的缩放倍率，详情请参考  [播放透明礼物特效](/real-time-video-ios-oc/other/play-transparent-gift-effects) 文档。
...
1. 支持前后摄像头同时采集视频
    ...
    相关 API 请参考 [setVideoSource](@setVideoSource), [useFrontCamera](@useFrontCamera), [ZegoVideoSourceType > ZegoVideoSourceTypeSecondaryCamera](@ZegoVideoSourceTypeSecondaryCamera), [ZegoVideoSourceType > ZegoVideoSourceTypeCamera](@ZegoVideoSourceTypeCamera)
...
2. 设置大流、小流的分辨率的 “比例” 需要保持一致，否则调用接口会出错。
    ...
    相关 API 请参考 [ZegoExpressEngine > setVideoConfig](@setVideoConfig), [setPublishDualStreamConfig](@setPublishDualStreamConfig)
...
1. 新增支持交付 XCFramework 产物，适配 Xcode 12.3
    ...
    请从开发者中心 [下载 SDK 包](./download-sdk.mdx) 选择下载 XCFramework。
