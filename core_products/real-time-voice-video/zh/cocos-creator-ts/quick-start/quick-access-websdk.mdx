...
以用户 A 和用户 B 互相推拉流为例，一次简单的实时音视频通话主要流程如下：
...
sequenceDiagram
 ...
 用户A->>ZEGO Express SDK: 3.用户A推流 <br/> startPublishingStream
 用户B->>ZEGO Express SDK: 4.用户B创建引擎 <br/> new ZegoExpressEngine
 用户B->>ZEGO Express SDK: 5.用户B登录房间 <br/> loginRoom
 用户B->>ZEGO Express SDK: 6.用户B推流 <br/> startPublishingStream
 ZEGO Express SDK->>用户B: 7.用户B收到roomStreamUpdate回调 <br/> (获取 用户A 的streamID)
 用户B->>ZEGO Express SDK: 8.用户B传入用户A的streamID拉流 <br/> startPlayStream
 ZEGO Express SDK->>用户A: 9.用户A收到roomStreamUpdate回调 <br/> (获取 用户B 的streamID)
 用户A->>ZEGO Express SDK: 10.用户A传入用户B的streamID拉流 <br/> startPlayStream
```
{/*
