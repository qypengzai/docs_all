...
回调场景示例：
- 场景一：用户 A 同时推流与拉流，远端用户只要有从 ZEGO 服务端拉 A 的流，就可以获取到用户 A 的上行网络质量与下行网络质量；用户 A 也会回调自身的上下行网络质量和用户 B 的下行网络质量。
- 场景二：用户 B 只推流不拉流，因为没有拉流，所以无法算出本端的下行网络质量，因此，远端用户只要有从 ZEGO 服务端拉 B 的流，就可以获取到用户 B 的上行网络质量，下行网络质量未知。在该场景下，用户 B 也只会回调自身的上行网络质量。
...
<td>2.22.0 及以上</td>
<<<<<<< HEAD
<td>基于 2.14.0 ～ 2.21.1 的 onNetworkQuality 接口的回调逻辑上，支持了两个网络质量状态回调：如果远端推流用户心跳丢失 1 次，回调其网络质量为 unknown；如果远端推流用户心跳丢失达到 3 次，回调其网络质量为 die。</td>
=======
<td>基于 2.14.0 ～ 2.21.1 的 onNetworkQuality 接口的回调逻辑上，如果远端推流用户心跳丢失 1 次，回调其网络质量为 unknown；如果远端推流用户心跳丢失达到 3 次，回调其网络质量为 die。</td>
>>>>>>> aacdd376b (RealTimeVoiceVideo: Update basic network quality report in monitor-stream-quality docs)
</tr>
<tr>
<td>2.14.0 ～ 2.21.1</td>
<td>
