...
调用 [updateCaptureRegion](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoScreenCaptureSource/updateCaptureRegion.html) 接口可以动态更新共享窗口区域。其中设置为 (0, 0, 0, 0) 可恢复整个窗口共享。
...
在屏幕共享时，您可以调用 [enableHightLight](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoScreenCaptureSource/enableHightLight.html) 接口对屏幕区域或者窗口启用描边，设置描边的颜色和宽度。
...
1. 调用 [callExperimentalAPI](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine/callExperimentalAPI.html) 接口安装插件
    - 参数值为 `{"method": "liveroom.audio.install_mac_virtual_audio_device", "params":{}}`
2. 监听 [onRecvExperimentalAPI](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine/onRecvExperimentalAPI.html) 回调安装结果：`{"method":"liveroom.audio.install_mac_virtual_audio_device","result":0}`，即安装成功

<Note title="说明">
- 只需要安装一次即可，重复安装会从 [onRecvExperimentalAPI](https://doc-zh.zego.im/unique-api/express-video-sdk/zh/dart_flutter/zego_express_engine/ZegoExpressEngine/onRecvExperimentalAPI.html) 接收到安装成功结果。
