...
用户 A 和用户 B 互相推拉流：双方各自创建引擎、登录房间、推流，然后收到房间内流变化通知后拉取对方的流。
...
sequenceDiagram
    ...
    用户A->>ZEGO Express SDK: 3.用户 A 推流
    用户B->>ZEGO Express SDK: 4.用户 B 创建引擎
    用户B->>ZEGO Express SDK: 5.用户 B 登录房间
    用户B->>ZEGO Express SDK: 6.用户 B 推流
    ZEGO Express SDK->>用户B: 7.用户 B 收到房间内流变化通知<br/>(获取用户 A 的 streamID)
    用户B->>ZEGO Express SDK: 8.用户 B 传入用户 A 的 streamID 拉流
    ZEGO Express SDK->>用户A: 9.用户 A 收到房间内流变化通知<br/>(获取用户 B 的 streamID)
    用户A->>ZEGO Express SDK: 10.用户 A 传入用户 B 的 streamID 拉流
```
{/*
