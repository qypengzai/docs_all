...
3. Media player supports caching resource files when playing online resources
    ...
    Developers can set the cache resource file path by configuring the [ZegoMediaPlayerResource-onlineResourceCachePath](@onlineResourceCachePath) parameter. Additionally, the [ZegoMediaPlayerResource-maxCachePendingLength](@maxCachePendingLength) parameter can be used to set the maximum pre-cacheable data length (in bytes) from the current playback time point. This allows the Media Player to reduce user resource download frequency and improve resource reuse rate and user experience when playing online resources. 
    <Warning title="Warning">
    ...
    For related API, please refer to [ZegoMediaPlayerResource > onlineResourceCachePath](@onlineResourceCachePath), [ZegoMediaPlayerResource > maxCachePendingLength](@maxCachePendingLength)
...
1. Media player supports Alpha data layout above the RGB data on the right side when playing transparent effects
    ...
    When the media player plays transparent effects, a new [ZegoAlphaLayoutType>ZegoAlphaLayoutTypeRightTop](@ZegoAlphaLayoutTypeRightTop) enumeration is added to support Alpha data being concatenated above the RGB data on the right side. When setting this enumeration, only a 0.5x zoom factor is supported.
...
1. Support simultaneous video capture from front and rear cameras.
    ...
    For related API, please refer to [setVideoSource](@setVideoSource), [useFrontCamera](@useFrontCamera), [ZegoVideoSourceType > ZegoVideoSourceTypeSecondaryCamera](@ZegoVideoSourceTypeSecondaryCamera), [ZegoVideoSourceType > ZegoVideoSourceTypeCamera](@ZegoVideoSourceTypeCamera)
...
2. Supports enabling voice changer effects for the audio output of the media player.
    ...
    For related API, please refer to [ZegoMediaPlayer > enableVoiceChanger](@enableVoiceChanger)
...
2. The ratio of setting the resolution for the big stream and the small stream needs to be consistent, otherwise it will cause errors when calling the interface. 
    ...
    For related API, please refer to [ZegoExpressEngine > setVideoConfig](@setVideoConfig), [setPublishDualStreamConfig](@setPublishDualStreamConfig)
...
3. Game voice function supports media player and audio player to use 3D sound effects.
    ...
    For related API, please refer to [ZegoMediaPlayer > updatePosition](@updatePosition), [ZegoAudioEffectPlayer > updatePosition](@updatePosition-ZegoAudioEffectPlayer)
...
5. Media player double speed function supports up to 4x.
    ...
    For related API, please refer to [ZegoMediaPlayer > setPlaySpeed](@setPlaySpeed)
...
2. Media player supports clearing the last frame when the playback ends.
    ...
    For related API, please refer to [ZegoMediaPlayer > clearView](@clearView)
...
5. The media player supports specifying the start playback progress when loading media resources.
    ...
    For related API, please refer to [ZegoMediaPlayer > loadResourceWithPosition](@loadResourceWithPosition)
...
3. The SDK package on the official website defaults to the media player that supports playing m3u8 format files.
    ...
    For related API, please refer to [ZegoMediaPlayer > loadResource](@loadResource)
...
4. Media player supports playing binary files.
    ...
    For related API, please refer to [ZegoMediaPlayer > loadResourceFromMediaData](@loadResourceFromMediaData)
...
1. The body player supports setting the maximum cache configuration of network resources and obtaining real-time network resource cache conditions.
    ...
    For related API, please refer to [ZegoMediaPlayer > setNetWorkResourceMaxCache](@setNetWorkResourceMaxCache), [ZegoMediaPlayer > getNetWorkResourceCache](@getNetWorkResourceCache)
...
2. Support setting the cache threshold that the media player needs to resume playback.
    ...
    For related API, please refer to [ZegoMediaPlayer > setNetWorkBufferThreshold](@setNetWorkBufferThreshold)
...
1. Media player added screenshot function.
    ...
    For related API, please refer to [ZegoMediaPlayer > takeSnapshot](@takeSnapshot)
...
5. Media player adds precise search function.
    ...
    For related API, please refer to [ZegoMediaPlayer > enableAccurateSeek](@enableAccurateSeek)
...
7. Delete [volume] and [getVolume] in [ZegoMediaPlayer], please use [setPublishVolume], [getPublishVolume], [setPlayVolume], [getPlayVolume] instead.
    ...
    For related API, please refer to [ZegoMediaPlayer > setPublishVolume](@setPublishVolume), [ZegoMediaPlayer > setPlayVolume](@setPlayVolume)
...
4. Media Player added support for setting voice changer effects.
    ...
    For related API, please refer to [ZegoMediaPlayer > setVoiceChangerParam](@setVoiceChangerParam)
...
7. Media Player added support for switching audio tracks.
    ...
    For related API, please refer to [ZegoMediaPlayer > setAudioTrackIndex](@setAudioTrackIndex)
