...
<tbody><tr>
<th>时刻</th>
<th style={{width:"20%"}}>onConnectionStateChanged<br />（网络状态回调</th>
...
<tr>
<td>`state` == 0（房间已断开）
<br />`event` == 2（其他原因）<br /></td>
<td>
其他原因断开连接，用户若需要重新进入房间，则可调用`joinRoom` 或 `enterRoom` 接口以重新加入房间，或在页面提示用户已退出房间聊天页面。
<Warning title="">此处不建议开发者进行自动重连。</Warning>
</td>
</tr>
</tbody></table>