...
- 在 AI 编程工具中设置对应的 Skills 并在给 AI 指定开发任务时指定使用该 Skills。
...
- 如果您使用的是 Claude Code，请按照我们提供的插件安装指南安装 [ZEGO Claude Code Plugins](https://github.com/ZEGOCLOUD/zego-claude-code-plugins)。

<Video src="https://doc-media.zego.im/general/zh/faq/zego-claude-code-plugin.mp4" />

- 如果您使用的是 DeepSeek Harness，请安装我们提供的 [dsh-zego-assistant](https://github.com/ZEGOCLOUD/dsh-zego-assistant) 插件。插件会自动配置 ZEGO 文档 MCP 服务并安装 ZEGO 相关 Skills，安装并重启对应的 profile 后，在"设置 > 插件 > 插件列表"中可以看到 zego-assistant 和 mcp-client 均处于"已启用"状态：
