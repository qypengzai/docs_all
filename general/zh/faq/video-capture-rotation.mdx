...
对于 v2.23.0 及以后版本，如需自定义视频采集旋转方式，为避免与其他视频朝向模式冲突，请勿调用 [setAppOrientationMode](https://doc-zh.zego.im/article/api?doc=Express_Video_SDK_API~objective-c_ios~class~ZegoExpressEngine#set-app-orientation-mode) 设置除自定义模式外的模式。 
...
以上三种方式调用的接口存在差异，详细描述请参考本文的 [实现流程](/faq/express_video_capture_rotation)。
